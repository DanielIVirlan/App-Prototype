...
struct RisultatoValutazioneView: View {
    ...
    var body: some View {
        ...
        .background(NavigationLink(destination: SceltaSmaltimentoView(), isActive: $vaiASmaltimento) { EmptyView() })
        .background(NavigationLink(destination: MainMenu(username: "Admin"), isActive: $tornaAlMenu) { EmptyView() })
    }
}
